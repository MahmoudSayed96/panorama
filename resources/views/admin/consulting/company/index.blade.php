...
<div class="row">
    <div class="col-12">
        ...
        <div class="tile">
            <h2 class="tile-title">استشارات هندسية للشركة</h2>
            ...
            @endif
            <div class="tile-body">
                @if (isset($cmpConsulting) && count($cmpConsulting) > 0)
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table class="table table-hover table-bordered dataTable no-footer text-center" id="dt"
                                    ...
                                    <tbody>
                                        @foreach ($cmpConsulting as $index=>$cmp)
                                        <tr role="row" class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">
                                            <td class="sorting_1">
                                                {{ $cmp->created_at->toDateString() }}</td>
                                            <td class="sorting_1">{{ $cmp->client_name }}</td>
                                            <td class="sorting_1">{{ $cmp->client_phone }}</td>
                                            <td class="sorting_1">{{ $cmp->project_name }}</td>
                                            <td class="sorting_1">{{ $cmp->project_number }}</td>
                                            <td class="sorting_1">{{ $cmp->piece_number }}</td>
                                            <td class="sorting_1">{{ $cmp->suk_number }}</td>
                                            <td>
                                                @if (currentUser()->hasPermission('read_consulting'))
                                                <a href="{{ route('admin.consulting.company.show',$cmp->id) }}"
                                                    class="btn btn-info btn-sm ml-3"><i class="fa fa-eye fa-lg"></i>
                                                    عرض</a>
                                                @endif
                                                @if (currentUser()->hasPermission('update_consulting'))
                                                <a href="{{ route('admin.consulting.company.edit',$cmp->id) }}"
                                                ...
                                                <form action="{{ route('admin.consulting.company.delete',$cmp->id) }}"
