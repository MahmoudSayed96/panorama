...
<div class="row">
    <div class="col-12">
        ...
        <div class="tile">
            <h2 class="tile-title">عملاء الأقساط والتمويل</h2>
            @if (currentUser()->hasPermission('create_investments'))
            <div class="tile-title">
                <a href="{{ route('admin.investments.premiums.create') }}" class="btn btn-success">اضاف جديد <i
                        class="fa fa-plus fa-fw fa-lg" aria-hidden="true"></i></a>
            </div>
            @endif
            <div class="tile-body">
                @if (isset($premiums) && count($premiums) > 0)
                                ...
                                <table class="table table-hover table-bordered dataTable no-footer text-center" id="dt"
                                    ...
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting_asc" tabindex="0" aria-controls="dt" rowspan="1"
                                                colspan="1" aria-sort="ascending"
                                                aria-label="تفاصيل القسط: activate to sort column descending">تفاصيل
                                                القسط</th>
                                            <th class="sorting_asc" tabindex="0" aria-controls="dt" rowspan="1"
                                                colspan="1" aria-sort="ascending"
                                                aria-label="مبلغ القسط: activate to sort column descending">مبلغ القسط
                                            </th>
                                            <th class="sorting_asc" tabindex="0" aria-controls="dt" rowspan="1"
                                                colspan="1" aria-sort="ascending"
                                                aria-label="اجمالى المبلغ المتبقى: activate to sort column descending">
                                                اجمالى المبلغ المتبقى
                                            </th>
                                            <th class="sorting_asc" tabindex="0" aria-controls="dt" rowspan="1"
                                                colspan="1" aria-sort="ascending"
                                                aria-label="تاريخ انتهاء القسط: activate to sort column descending"
                                                style="width:60px;">
                                                تاريخ انتهاء القسط</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr role="row" class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">
                                            <td class="sorting_1">
                                                {{ $premium->created_at->toDateString() }}</td>
                                            <td class="sorting_1">{{ $premium->client_name }}</td>
                                            <td class="sorting_1">{{ $premium->client_phone }}</td>
                                            <td class="sorting_1">{!! $premium->details !!}</td>
                                            <td class="sorting_1">{{ formatNumber($premium->alqist_amount) }}</td>
                                            <td class="sorting_1">{{ formatNumber($premium->remaining_amount) }}</td>
                                            <td class="sorting_1">{{ $premium->end_amount_date }}</td>
                                            <td>
                                                @if (currentUser()->hasPermission('update_investments'))
                                                <a href="{{ route('admin.investments.premiums.edit',$premium->id) }}"
                                                    class="btn btn-warning btn-sm"><i
                                                        class="fa fa-pencil-square fa-lg"></i> تعديل</a>
                                                @endif
                                                @if (currentUser()->hasPermission('delete_investments'))
                                                <form
                                                    action="{{ route('admin.investments.premiums.delete',$premium->id) }}"
