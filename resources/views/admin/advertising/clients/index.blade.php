...
<div class="row">
    <div class="col-12">
        ...
        <div class="tile">
            <h2 class="tile-title">اعلانات خارجية</h2>
            @if (currentUser()->hasPermission('create_decorations'))
            <div class="tile-title">
                <a href="{{ route('admin.advertising.clients.create') }}" class="btn btn-success">اضاف جديد <i
                        class="fa fa-plus fa-fw fa-lg" aria-hidden="true"></i></a>
            </div>
            @endif
            <div class="tile-body">
                @if (isset($clientDesigns) && count($clientDesigns) > 0)
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table class="table table-hover table-bordered dataTable no-footer text-center" id="dt"
                                    ...
                                    <tbody>
                                        @foreach ($clientDesigns as $index=>$client)
                                        <tr role="row" class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">
                                            ...
                                            <td class="sorting_1">{{ $client->delivered_date }}</td>
                                            <td>
                                                @if (currentUser()->hasPermission('read_decorations'))
                                                <a href="{{ route('admin.advertising.clients.show',$client->id) }}"
                                                    class="btn btn-info btn-sm ml-3"><i class="fa fa-eye fa-lg"></i>
                                                    عرض</a>
                                                @endif
                                                @if (currentUser()->hasPermission('update_decorations'))
                                                <a href="{{ route('admin.advertising.clients.edit',$client->id) }}"
                                                ...
                                                <form
                                                    action="{{ route('admin.advertising.clients.delete',$client->id) }}"
