...
<div class="row">
    <div class="col-12">
        ...
        <div class="tile">
            <h2 class="tile-title">ديكورات الشركة</h2>
            @if (currentUser()->hasPermission('create_decorations'))
            <div class="tile-title">
                <a href="{{ route('admin.decorations.company.create') }}" class="btn btn-success">اضاف جديد <i
                        class="fa fa-plus fa-fw fa-lg" aria-hidden="true"></i></a>
            </div>
            @endif
            <div class="tile-body">
                @if (isset($cmpDecorations) && count($cmpDecorations) > 0)
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table class="table table-hover table-bordered dataTable no-footer text-center" id="dt"
                                    ...
                                    <tbody>
                                        @foreach ($cmpDecorations as $index=>$cmp)
                                        <tr role="row" class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">
                                            <td class="sorting_1">
                                                {{ $cmp->created_at->toDateString() }}</td>
                                            <td class="sorting_1">{{ $cmp->client_name }}</td>
                                            <td class="sorting_1">{{ $cmp->client_phone }}</td>
                                            <td class="sorting_1">{{ formatNumber($cmp->paid_amount) }}</td>
                                            <td class="sorting_1">{{ $cmp->delivered_date }}</td>
                                            <td>
                                                @if (currentUser()->hasPermission('read_decorations'))
                                                <a href="{{ route('admin.decorations.company.show',$cmp->id) }}"
                                                    class="btn btn-info btn-sm ml-3"><i class="fa fa-eye fa-lg"></i>
                                                    عرض</a>
                                                @endif
                                                @if (currentUser()->hasPermission('update_decorations'))
                                                <a href="{{ route('admin.decorations.company.edit',$cmp->id) }}"
                                                ...
                                                <form action="{{ route('admin.decorations.company.delete',$cmp->id) }}"
