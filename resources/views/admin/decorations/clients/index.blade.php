...
<div class="app-title">
    ...
    <ul class="app-breadcrumb breadcrumb">
        ...
        <li class="breadcrumb-item active">الديكور والاعلان</li>
    </ul>
</div>
<div class="row">
    <div class="col-12">
        ...
        <div class="tile">
            <h2 class="tile-title">ديكورات عملاء اخرين</h2>
            @if (currentUser()->hasPermission('create_decorations'))
            <div class="tile-title">
                <a href="{{ route('admin.decorations.clients.create') }}" class="btn btn-success">اضاف جديد <i
                        class="fa fa-plus fa-fw fa-lg" aria-hidden="true"></i></a>
            </div>
            @endif
            <div class="tile-body">
                @if (isset($clientDecorations) && count($clientDecorations) > 0)
                                ...
                                <table class="table table-hover table-bordered dataTable no-footer text-center" id="dt"
                                    ...
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting_asc" tabindex="0" aria-controls="dt" rowspan="1"
                                                colspan="1" aria-sort="ascending"
                                                aria-label="تاريخ الاستلام: activate to sort column descending">تاريخ
                                                الاستلام</th>
                                            <th>التحكم</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($clientDecorations as $index=>$clientDoc)
                                        <tr role="row" class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">
                                            <td class="sorting_1">
                                                {{ $clientDoc->created_at->toDateString() }}</td>
                                            <td class="sorting_1">{{ $clientDoc->client_name }}</td>
                                            <td class="sorting_1">{{ $clientDoc->client_phone }}</td>
                                            <td class="sorting_1">{{ formatNumber($clientDoc->paid_amount) }}</td>
                                            <td class="sorting_1">{{ $clientDoc->delivered_date }}</td>
                                            <td>
                                                @if (currentUser()->hasPermission('read_decorations'))
                                                <a href="{{ route('admin.decorations.clients.show',$clientDoc->id) }}"
                                                    class="btn btn-info btn-sm ml-3"><i class="fa fa-eye fa-lg"></i>
                                                    عرض</a>
                                                @endif
                                                @if (currentUser()->hasPermission('update_decorations'))
                                                <a href="{{ route('admin.decorations.clients.edit',$clientDoc->id) }}"
                                                    class="btn btn-warning btn-sm ml-3"><i
                                                        class="fa fa-pencil-square fa-lg"></i> تعديل</a>
                                                @endif
                                                @if (currentUser()->hasPermission('delete_decorations'))
                                                <form
                                                    action="{{ route('admin.decorations.clients.delete',$clientDoc->id) }}"
